...
export default function Home() {
  ...
  useEffect(() => {
    if (isLoggedIn && IdStore.recruiterId) {
      setFormsLoading(true);
      fetchData();
    } else {
      setFormsLoading(false);
    }
  }, [isLoggedIn, IdStore.recruiterId]);

  const handleLogout = async () => {
    try {
      setLoggingOut(true);
      await logoutUser();
      // AuthWrapper's onAuthStateChanged listener picks this up,
      // clears Redux user state, and redirects as needed.
    } catch (error) {
    ...
    <main>
      <div className="max-w-[800px] m-auto">
        <div className="m-5">
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-3 shrink-0">
                <UserButton
                  ...
                  name={
                    currentUser?.displayName || currentUser?.email || "Recruiter"
                  }
          ...
          {!isLoggedIn ? (
            <Paper className="p-10 rounded-md shadow-md text-center">
              <Typography order={3}>Welcome to the Recruiter Portal</Typography>
              <Typography order={5} className="font-light mt-2">
                Log in if you already have an account, or register if this is
                your first time hiring from IIT BHU.
              </Typography>
